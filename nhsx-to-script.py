...
def generate_transcript(xml_file, output_file=None):
...
if __name__ == "__main__":
    if len(sys.argv) != 2 and len(sys.argv) != 3:
        print("Usage: python hindenburg_transcript.py <xml_file> [output_file]")
        sys.exit(1)

    xml_filepath = sys.argv[1]
    output_filepath = sys.argv[2] if len(sys.argv) == 3 else os.path.splitext(xml_filepath)[0] + ".md"
    generate_transcript(xml_filepath, output_filepath)